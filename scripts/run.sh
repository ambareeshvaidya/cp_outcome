...
for i in $CUTS; do 
  ...
  for j in $RANDOM_SEED; do
    ...
    mkdir -p /blue/akazachkov/ambareesh.vaidya/cp_outcome/paper/results/cut_${CUT_MODE}/seed_${SEED}/${CASE_NUM}
    INSTANCE_FILE=${TYPE}.instances
    RESULT_DIR="/blue/akazachkov/ambareesh.vaidya/cp_outcome/paper/results//cut_${CUT_MODE}/seed_${SEED}/${CASE_NUM}"
    ...
    python /home/ambareesh.vaidya/repos/cp_outcome/src/fullrun_new.py --instance ${FILE} --cuts ${CUT_MODE} --result_path ${RESULT_DIR} --seed ${SEED}
